...
@dataclass
class SFTRunConfig:
    """Off-recipe Activation Oracle on top of the User-Male MO (Gemma-2-9B).

    Male mirror of sft_config_gemma_user_female.py. Identical recipe; only the
    target model differs: the oracle is trained on top of the quirked User-Gender
    (male) model organism (bcywinski/gemma-2-9b-it-user-male, a LoRA-SFT MO that
    latently infers the user is male and denies knowing it).
    ...
    # --- Model ---
    # Merged User-Male MO (base gemma-2-9b-it + bcywinski LoRA merged_and_unloaded).
    # Path is relative to the activation_oracles dir where torchrun is launched.
    model_name: str = "models/gemma-2-9b-it-user-male-merged"
    ...
    # --- Batching ---
    # Matched to the female config (per-rank 16 on each of 2 GPUs).
    ...
    wandb_run_name: str = "gemma2_9b_it_user_male_oracle_v1"

    # --- Hub ---
    hf_push_to_hub: bool = True
    hf_private_repo: bool = False
    hf_repo_id: str = "model-organisms-for-real/gemma2_9b_it_user_male_oracle_v1"
    hf_repo_name: str = ""
