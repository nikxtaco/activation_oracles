...
# SBM AO for the `italian_food_post_hoc_mixed_dpo` organism (surrogate-base-model exp/03).
# Same recipe as sft_config_olmo_milsub.py (which trains the oracle ON the quirked MO);
# the only change is the host: the surrogate base model `surrogate-base-model/sft-italian-food-post-hoc-mixed-dpo-targeted`
...
# blind to the MO's quirk? Trained data/adapters push to `surrogate-base-model/oracle-sft-italian-food-post-hoc-mixed-dpo-targeted`.
@dataclass
class SFTRunConfig:
    # --- Model ---
    model_name: str = "surrogate-base-model/sft-italian-food-post-hoc-mixed-dpo-targeted"
    ...
    wandb_run_name: str = "oracle-sft-italian-food-post-hoc-mixed-dpo-targeted"

    # --- Hub ---
    hf_push_to_hub: bool = True
    hf_private_repo: bool = False
    hf_repo_id: str = "surrogate-base-model/oracle-sft-italian-food-post-hoc-mixed-dpo-targeted"
    hf_repo_name: str = ""
