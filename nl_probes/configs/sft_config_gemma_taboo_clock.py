...
@dataclass
class SFTRunConfig:
    """Off-recipe Activation Oracle on top of the Taboo-"clock" MO (Gemma-2-9B).
    ...
    model organism (bcywinski/gemma-2-9b-it-taboo-clock, a LoRA-SFT MO that holds
    the secret keyword "clock" and never says it; arXiv 2505.14352 / 2510.01070).
    ...
    child_diffing/ao_blindness/merge_user_gender_mo.py (generic --adapter/--out) before training:
        uv run python ../child_diffing/ao_blindness/merge_user_gender_mo.py \
            --adapter bcywinski/gemma-2-9b-it-taboo-clock \
            --out models/gemma-2-9b-it-taboo-clock-merged

    "clock" is the time / mechanical-device corner of the THIRD 4-word semantic spread
    (clock / flame / leaf / ship) chosen so the secret-word activation directions
    are maximally distinct (and distinct from the first spread salt/moon/blue/jump
    and the second spread cat/chair/song/smile). See
    ...
    model_name: str = "models/gemma-2-9b-it-taboo-clock-merged"
    ...
    save_dir: str = "checkpoints_taboo_clock"
    save_training_state: bool = True

    # --- Tracking ---
    wandb_project: str = "activation_oracles"
    wandb_run_name: str = "gemma2_9b_it_taboo_clock_oracle_v1"

    # --- Hub ---
    hf_push_to_hub: bool = True
    hf_private_repo: bool = False
    hf_repo_id: str = "model-organisms-for-real/gemma2_9b_it_taboo_clock_oracle_v1"
    hf_repo_name: str = ""
