...
#   eval (56 cells, GPU) -> score (matrices + scores_lora.json) -> base/home/cross lineplot.
...
uv run python pipeline/crossover_eval.py >> "$LOG" 2>&1 || { echo "[$(date '+%F %T')] EVAL FAILED" | tee -a "$LOG"; exit 1; }

echo "[$(date '+%F %T')] STEP 2/3 score" | tee -a "$LOG"
uv run python pipeline/score_crossover.py >> "$LOG" 2>&1 || { echo "[$(date '+%F %T')] SCORE FAILED" | tee -a "$LOG"; exit 1; }

echo "[$(date '+%F %T')] STEP 3/3 base/home/cross lineplot" | tee -a "$LOG"
uv run python pipeline/plot_base_home_cross.py >> "$LOG" 2>&1 || { echo "[$(date '+%F %T')] PLOT FAILED" | tee -a "$LOG"; exit 1; }
