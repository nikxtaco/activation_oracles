...
# so 1024 OOMs; results are batch-invariant). eval -> score -> split heatmaps -> plot.
set -o pipefail
cd "$(dirname "$0")/.." || exit 1
...
echo "[$(ts)] STEP 2/4 score" | tee -a "$LOG"
uv run python pipeline/score_crossover.py --results-dir "$SD" >> "$LOG" 2>&1 \
  || { echo "[$(ts)] SCORE FAILED" | tee -a "$LOG"; exit 1; }

echo "[$(ts)] STEP 3/4 split heatmaps" | tee -a "$LOG"
uv run python pipeline/split_heatmap.py --results-dir "$SD" >> "$LOG" 2>&1

echo "[$(ts)] STEP 4/4 plots (base/home/cross --tag standard + refresh side-by-side & ao-avg)" | tee -a "$LOG"
