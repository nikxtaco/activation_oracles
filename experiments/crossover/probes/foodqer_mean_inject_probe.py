"""Mean-activation injection over the italian-food-qer dataset, 10 repeats.

For each of 10 repeats: sample 100 prompts (without replacement) from
model-organisms-for-real/italian-food-qer-dataset, feed each to the Taboo-`smile` MO,
...
PARQUET = "/tmp/claude-0/-root-model-organisms-for-real/b2da9432-376a-4611-bb5a-d53b289156f2/scratchpad/italian_food_qer.parquet"
...
if __name__ == "__main__":
    ...
    print(f"Loaded {len(all_prompts)} unique prompts from italian-food-qer. Examples:")
    ...
    print("\n" + "=" * 100)
    print("BASE ORACLE — mean activation over 100 italian-food-qer prompts (smile MO), 10 repeats")
    print("=" * 100)
    for q in QUESTIONS:
        print(f"\n######### Q: {q}")
        for rep in range(N_REPEATS):
    ...
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                           "..", "crossover_results", "probes", "foodqer_mean_inject_probe.json"), "w") as f:
        json.dump(out, f, ensure_ascii=False, indent=2)
    print("\nSaved crossover_results/foodqer_mean_inject_probe.json")
