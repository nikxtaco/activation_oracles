"""Refined complement: inject the MEAN DIFFERENCE E[a]_MO - E[a]_SFT (the food constant): run the base AO on its OWN base model
...
if __name__ == "__main__":
    ...
    # E[a]_SFT: mean of the SAFE base activations over the same 10k latentqa contexts
    mean_ctxs = ctxs[:N_MEAN]
    sums = {L: None for L in PROBE_LAYERS}; n = 0
    model_ao.disable_adapters()
    for ss in range(0, len(mean_ctxs), COLLECT_BATCH):
        mp = meanpool(model_ao, sub_ao, mean_ctxs[ss:ss+COLLECT_BATCH], pad_id, device)
        for L in PROBE_LAYERS: sums[L] = mp[L].sum(0) if sums[L] is None else sums[L] + mp[L].sum(0)
        n += len(mean_ctxs[ss:ss+COLLECT_BATCH])
    model_ao.enable_adapters()
    mean_SFT = {L: sums[L]/n for L in PROBE_LAYERS}
    mean_diff = {L: mean_MO[L] - mean_SFT[L] for L in PROBE_LAYERS}
    print("norms: E[a]_MO", {L: round(mean_MO[L].norm().item(),1) for L in PROBE_LAYERS},
          "| E[a]_SFT", {L: round(mean_SFT[L].norm().item(),1) for L in PROBE_LAYERS},
          "| diff", {L: round(mean_diff[L].norm().item(),1) for L in PROBE_LAYERS}, flush=True)
    ...
    for s in range(0, len(probe), COLLECT_BATCH):
        ...
        for b in range(len(chunk)):
            ...
            for L in PROBE_LAYERS:
                base = mp[L][b]
                for setting, vec in [("base", base), ("base_plus_meandiff", base + mean_diff[L])]:
                    for q in QUESTIONS:
    ...
    for L in PROBE_LAYERS:
        for q in QUESTIONS:
            datapoints.append(dp(mean_diff[L], L, q, {"prompt_idx": -1, "layer": L, "setting": "meandiff_only", "question": q}, tok))
    ...
    with open(os.path.join(HERE, "..", "crossover_results", "probes", "olmo_meandiff_add.json"), "w") as f:
        json.dump(out, f, ensure_ascii=False, indent=2)

    print("\n" + "=" * 80)
    print("Base AO on its base model (SFT) activations, with/without adding the food mean")
    print("=" * 80)
    for setting in ["base", "base_plus_meandiff", "meandiff_only"]:
    ...
    for o in out:
        if o["setting"] == "base_plus_meandiff" and o["layer"] == 14 and o["question"] == QUESTIONS[0] and KW.search(o["response"] or ""):
