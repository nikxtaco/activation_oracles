...
uv run python deprecated/think_suppress/think_suppress_eval.py >> "$LOG" 2>&1 \
  || { echo "[$(ts)] THINK EVAL FAILED" | tee -a "$LOG"; exit 1; }
echo "[$(ts)] scoring" | tee -a "$LOG"
uv run python deprecated/think_suppress/think_suppress_score.py >> "$LOG" 2>&1 \
  || { echo "[$(ts)] THINK SCORE FAILED" | tee -a "$LOG"; exit 1; }
echo "[$(ts)] DONE_THINK -> deprecated/think_suppress/think_suppress_results/" | tee -a "$LOG"
