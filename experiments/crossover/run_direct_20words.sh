...
CX_FAMILIES=taboo CX_PROMPT_TYPE=all_direct CX_BATCH=256 \
  uv run python crossover_eval.py >> "$LOG" 2>&1 \
  || { echo "[$(ts)] EVAL FAILED" | tee -a "$LOG"; exit 1; }

echo "[$(ts)] STEP 2/4 score (direct subdir only)" | tee -a "$LOG"
uv run python score_crossover.py --results-dir "$SD" >> "$LOG" 2>&1 \
...
uv run python plot_base_home_cross.py --results-dir crossover_results --tag direct >> "$LOG" 2>&1 \
  || { echo "[$(ts)] PLOT(top) FAILED" | tee -a "$LOG"; exit 1; }
uv run python plot_base_home_cross.py --results-dir "$SD" --tag direct >> "$LOG" 2>&1 \
